            ...
            <div className='w-full pt-16 md:pt-0 md:pl-36'>
                <div className='flex flex-col md:flex-row items-start justify-between'>
                    <div>
                        ...
                        <p>{user.username ? `@${user.username}` : 'Thêm tên người dùng'}</p>
                    </div>
                    {/* nếu người dùng không có trong hồ sơ của người khác nghĩa là họ đang mở hồ sơ của mình nên chúng tôi sẽ cung cấp nút chỉnh sửa */}
                    {
                    !profileId && 
                ...
                <div className='flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-gray-500 mt-4'>
                    <span className='flex items-center gap-1.5'>
                        ...
                        {user.location ? user.location : 'Thêm Địa Chỉ'}
                    </span>
                    <span className='flex items-center gap-1.5'>
                        <Calendar className='w-4 h-4'/>
                        Tham Gia <span className='font-medium'>{moment(user.createAt).fromNow()}</span>
                    </span>
                </div>  
